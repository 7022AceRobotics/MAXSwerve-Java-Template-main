...
public class driveToCoralStationRight extends Command {
  ...
  public driveToCoralStationRight(DriveSubsystem m_drive_subsystem, PhotonVisionHelper m_photon_vision_subsystem) {
  ...
  public void initialize() {
    ...
    pose_initial = new Pose2d(pose_initial.getX(), pose_initial.getY(),
    new Rotation2d(pose_initial.getRotation().getRadians() + Math.PI));
    pose_final = position_of_apriltag.rotateBy(position_of_apriltag.getRotation()
    .times(-1)).transformBy(new Transform2d(DriveConstants.kWheelBase/2, -0.08, 
    new Rotation2d(0))).rotateBy(position_of_apriltag.getRotation());
